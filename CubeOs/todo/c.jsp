...
<%! public double[] ecefToLla( double x, double y, double z) {
 ...
 String sql="";
<%! public long crunch (long cube, long idCt, double[] pos, double cubeLen) {
    idCt++;
 sql+=","+cube;
 for(int x=0;x<2;x++) {
  for(int y=0;y<2;y++) {
     ...
     for(int yi=0;yi<2;yi++) {
      for(int zi=0;zi<2;zi++) {
       double alt= ecefToLla(
        ...
        pos[2]+cubeLen/2*z+cubeLen/2*zi)[2]; 
       allCrust=allCrust && alt<0;
      }
     }
    }
    if (!allCrust && cubeLen > 1000024) {
     double[] newPos={
       pos[0]+cubeLen/2*x,
       pos[1]+cubeLen/2*y,
       pos[2]+cubeLen/2*z};
     idCt=crunch(idCt,idCt+1,newPos,cubeLen/2);
    }
   }
  }
 }
