...
//log(root);
 ...
 try { 
  ...
  xBox=document.getElementById('xBox').getContext('2d');
  yBox=document.getElementById('yBox').getContext('2d');
  zBox=document.getElementById('zBox').getContext('2d');
 } catch (e){};
 //const
 ...
 */
 ...
 function remoteGet() {
  ...
  request.send('/drive', 'POST', data.toString());
  return false;
 }
...
/* function rotX(r) {
  tmpMat=[
   [rotMat[0][0], rotMat[1], rotMat[2]],
   ];
  rotMat=tmpMat;
 }*/

 function rotColName(x,y,z) {
  var v=vecMult(rotMat,[x*2-1,y*2-1,z*2-1])
  c="c";
  c+=(v[0]+1)/2;
  c+=(v[1]+1)/2;
  c+=(v[2]+1)/2;
...
/*  var cName='c'+xi+''+yi+''+zi;
 ...
 function keyHandler(event) {
  ...
  if (event.keyCode==82) { //'r'
   if (event.shiftKey) {
    rotMat=matMult(rotMat,rotY);
    rotMat=matMult(rotMat,rotY);
    rotMat=matMult(rotMat,rotY);
    //rotZ=(rotZ+3)%4;
   } else {
    rotMat=matMult(rotMat,rotZ);
    rotMat=matMult(rotMat,rotZ);
    rotMat=matMult(rotMat,rotZ);
    //rotZ=(rotZ+1)%4;
   }
 ...
 function arrowedLine(x0,y0,x1,y1,s) {
  ctx.strokeStyle=s;
  var dx=x1-x0;
  var dy=y1-y0;
  ...
  for(var i=1;i<7;i++) {
   ctx.lineWidth=i;
   ctx.beginPath();
   ctx.moveTo(x0+ i   *dx/len,y0+ i   *dy/len);
   ctx.lineTo(x0+(i+1)*dx/len,y0+(i+1)*dy/len);
   ctx.stroke();
  }
  for(var i=6;i>0;i--) {
   ctx.lineWidth=i;
   ctx.beginPath();
   ctx.moveTo(x1- i   *dx/len,y1- i   *dy/len);
   ctx.lineTo(x1-(i+1)*dx/len,y1-(i+1)*dy/len);
   ctx.stroke();
  }
  ctx.lineWidth=2;
  ctx.beginPath();
 ...
 function drawCoordinates() {
  ...
  ctx.fillStyle='#f00';
  ctx.strokeStyle='#f00';
  ctx.fillText('x',20,104);
  arrowedLine(25,110,80,55,'#f00');
  arrowedArc(30,110,30,5,7,true);
  arrowedArc(30,105,30,1,3,false);

  ctx.fillStyle='#0f0';
  ctx.strokeStyle='#0f0';
  ctx.fillText('y',112,83);
  arrowedLine(35,70,120,70,'#0f0');
  arrowedArc(95,66,40,7,1,false);
  arrowedArc(135,76,40,3,5,true);
  
  ctx.fillStyle='#00f';
  ctx.strokeStyle='#00f';
  ctx.fillText('z',70,33);
  arrowedLine(65,100,65,25,'#00f');
  arrowedArc(71,49,40,5,7,true);
  arrowedArc(65,7,40,1,3,false);
  ctx.shadowOffsetX=0;
  ctx.shadowOffsetY=0;
 }
